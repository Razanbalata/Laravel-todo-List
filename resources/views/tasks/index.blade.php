            ...
            <div class="col-span-12 lg:col-span-8 space-y-4">
                ...
                @empty
                    <div class="col-span-12">
                        <div class="bg-white border border-outline-variant rounded-xl p-10 text-center">

                            @if (request('category') && request('priority'))
                                <h3 class="text-xl font-bold">
                                    No tasks found in {{ request('category') }}
                                    with {{ request('priority') }} priority
                                </h3>
                            @elseif(request('category'))
                                <h3 class="text-xl font-bold">
                                    No tasks found in {{ request('category') }} category
                                </h3>
                            @elseif(request('priority'))
                                <h3 class="text-xl font-bold">
                                    No tasks found with {{ request('priority') }} priority
                                </h3>
                            @else
                                <h3 class="text-xl font-bold">
                                    No tasks available
