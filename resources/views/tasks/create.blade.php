            ...
            <form action="{{ $action ?? route('tasks.store') }}" method="POST" class="space-y-stack-lg">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-stack-lg">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-3 bg-surface-container rounded-xl p-1 gap-1">
                            ...
                            <button type="button" id="btn-high" onclick="setPriority(1,'high')"
                                class="py-2 rounded-lg text-sm font-medium transition-all">
                                High
                            </button><input value="{{ old('priority_id', $task->priority_id ?? '') }}" type="hidden"
    ...
    <script>
        function setPriority(id, level) {
            ...
            Object.values(btns).forEach(btn => {
                btn.className =
                    "py-2 rounded-lg text-sm font-medium text-on-surface-variant transition-all";
            });
            ...
            if (level === 'low') {
                active.className += " bg-green-100 text-green-700 shadow";
            }

            if (level === 'medium') {
                active.className += " bg-yellow-100 text-yellow-700 shadow";
            }

            if (level === 'high') {
                active.className += " bg-red-100 text-red-700 shadow";
            }
        }
